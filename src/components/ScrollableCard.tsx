...
const ScrollableCard = ({ title, isElden = false, children }: { title: string; isElden?: boolean; children: React.ReactNode }) => {
    ...
    const height = isElden ? 'h-[20rem]' : 'h-[46rem]';

    return (
        <ScrollArea type="always" ref={contentRef} onScrollCapture={checkScroll} className={`w-full ${height}`}>
            <div
